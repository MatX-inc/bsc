 ...
 public:
                              ...
                              const char* name = NULL)
  {
    ...
    for (unsigned int i = 0; i < ports; i++) {
      ...
      vcd_write_def(sim_hdl, num++, buf, bits, WAVE_OUTPUT,
                    wave_port_type(port_types, buf));

      snprintf(buf, sizeof(buf), "EN_%u", i);
      vcd_set_clock(sim_hdl, num, __clk_handle_0);
      vcd_write_def(sim_hdl, num++, buf, 1, WAVE_INPUT, "Bool");

      snprintf(buf, sizeof(buf), "D_IN_%u", i);
      vcd_set_clock(sim_hdl, num, __clk_handle_0);
      vcd_write_def(sim_hdl, num++, buf, bits, WAVE_INPUT,
                    wave_port_type(port_types, buf));
    }
    vcd_write_scope_end(sim_hdl);
