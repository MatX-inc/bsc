...
Every failure is classified so the output is a work list, not a score:
  ...
  EXPORT_FAIL    SimExportIR internalError (unhandled IR construct);
                 a DECLINED design is NOT_SUPPORTED, not this
...
def one_test(job):
    ...
    if rb is None or rb.returncode != 0 or not os.path.exists(bir):
        ...
        return (rel, top, _export_class(msg),
...
def _export_class(msg):
    """EXPORT_FAIL means the exporter broke; a refusal is not that.

    S0015 is the BVI refusal suite declining a contract outside the
    accepted set -- a statement about the design, reported to the user,
    and the same answer NOT_SUPPORTED already carries for a design
    reference Bluesim will not run either.  Leaving it in EXPORT_FAIL
    makes a working refusal read as an exporter bug.
    """
    return "NOT_SUPPORTED" if "(S0015)" in msg else "EXPORT_FAIL"
