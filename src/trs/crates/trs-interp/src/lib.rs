...
impl Interp {
                ...
                ir::InstanceKind::Bvi(c) => {
                                            ...
                                            BviParamRefKind::Str => {
                                                let sid = match e {
                                                    ...
                                                    _ => {
                                                        ...
                                                        self.eval(slot, &mut cx, e).as_str_id()
                                                    }
                                                };
                                                ...
                                                trs_vlt::ResolvedParam::Str(self.s(sid).to_string())
                                            }
                    ...
                    for (ord, oc) in c.out_clocks.iter().enumerate() {
                        ...
                        self.dynclk_init.insert(
                            ...
                            (clk_out_init.get(ord).copied().unwrap_or(false), ord as u32),
                        );
                    }
    ...
    fn derive_plan_a(&mut self) -> PlanA {
        ...
        self.bvi_clk_ci = sources
            ...
            .enumerate()
            .filter_map(|(ci, s)| match s {
                ClockSource::Triggered { driver, ord, .. } => Some(((*driver, *ord), ci)),
                _ => None,
            })
            .collect();
    ...
    pub fn prime(&mut self) {
        ...
        for (ci, &c) in clocks.iter().enumerate() {
            ...
            if !self.dynclk_init.contains_key(&cname) && !cname.ends_with("$CLK_OUT") {
                continue;
            }
        ...
        for idx in self.bvi_insts.clone() {
            let Some(&node) = self.rstgen_out.get(&idx) else {
                continue;
            };
    ...
    pub fn advance_until(&mut self, cond: &StopCond) -> i32 {
        ...
        while self.fe.finished.is_none() && !self.fe.stop_request {
            ...
            if !same_time {
                ...
                same_time = matches!(heap.peek(), Some(Reverse((nt, _, _, _))) if *nt == t);
            }
            if !same_time {
                self.flush_reset_pending();
