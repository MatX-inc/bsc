...
renumberModule vm =
    let
        ...
        instFixed (VMInst m i pas pos) =
            [vidStr m, vidStr i] ++
            [ x | VId x _ _ <- listify isVId (map fst pas) ] ++
            map (vidStr . fst) pos
        ...
        -- foreign linkage names must never change: DPI import
        -- declarations and the function/task names of foreign calls
        ...
        foreignFixed :: [String]
        foreignFixed =
            [ s | VMDPI dpi <- listify (\it -> case it of
                                                 VMDPI _ -> True
                                                 _       -> False)
                                       (vm_body vm)
                , VId s _ _ <- listify isVId dpi ] ++
