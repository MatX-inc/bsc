...
--    trace ("mkInstId " ++ ppReadable (mi,t, expandSyn t)) $
  ...
  where
        pos = getPosition t
        qfs = getIdFString mi
        inst_fs = concatFString (intersperse fsTilde (map getIdFStringP (flat (expandSyn M.empty t))))
...
chkTopDef r mi src_pkg isDep (Cforeign i qt on ops ni) = do
    ...
    if ni || isGoodType (expandSyn (getATFEqs r) t) then
...
mkTypeSyms errh mkQuals maybePackageName src_pkg iks defs qts s =
    let importedTypeInfos0 = concatMap (getTI errh maybePackageName src_pkg r iks) defs
        (cls, errss) =
            unzip $
...
getTI _ mi src_pkg _ iks (Cstruct _ ss ik vs fs _) =
  ...
  where i = qual mi (iKName ik)
getTI _ mi src_pkg _ iks (Cclass _ ps ik vs _ ats fs) =
    (i, TypeInfo (Just i) k vs ti src_pkg) : atf_tis
  where i = qual mi (iKName ik)
        k = getK iks ik
        ...
        atf_tis = [ (atf_i, TypeInfo (Just atf_i) atf_k vs (TIatf n_total) src_pkg)
...
getTI _ mi src_pkg _ iks (CItype ik vs _) =
  ...
  where i = qual mi (iKName ik)
getTI _ mi src_pkg _ iks (CIclass _ ps ik vs _ _ _) =
    [(i, TypeInfo (Just i) k vs ti src_pkg)]
  where i = qual mi (iKName ik)
        k = getK iks ik
