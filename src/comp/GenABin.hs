...
header :: [Byte]
header = B.unpack $ TE.encodeUtf8 $ T.pack "bsc-ba-20260702-1"
...
instance Bin VMItem where
    ...
    writeBytes (VMIfDef m ts es)    = do putI 8; toBin m; toBin ts; toBin es
    readBytes = do
      i <- getI
      case i of
        ...
        8 -> do m <- fromBin; ts <- fromBin; es <- fromBin;
                return (VMIfDef m ts es)
