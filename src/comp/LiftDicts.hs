...
adoptDict (CValueSign (CDefT i [] (CQType [] t) [CClause [] [] e])) = do
  ...
  modify (\s -> s {
      ...
      -- the pre-conversion pool too, so a later candidate with the
      -- same (type, evidence) hits without converting (the
      -- typechecker's evidence is emitted substitution-applied and
      -- nullary-application-peeled, the same form handleDict pools)
      dictPoolC = M.insertWith (\new old -> old ++ new) t [(e, i)] (dictPoolC s),
