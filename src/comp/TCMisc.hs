...
reducePred eps dvs vp = do
    ...
    case mhit of
      Just b -> return (Just ([], mkSolvedBind b False, nullSubst,
                              Nothing, Nothing))
      Nothing -> reducePredViaInstances eps dvs vp

reducePredViaInstances :: [EPred] -> DVS -> VPred ->
                          TI (Maybe ([VPred], SolvedBind, Subst, Maybe Pred, Maybe Id))
reducePredViaInstances eps dvs (VPred w pp@(PredWithPositions pr@(IsIn c ts) pos)) = do
    pushSatStackContext
    bound_tyvars <- getBoundTVs
    ts' <- mapM normT ts
    let pr' = IsIn c ts'
