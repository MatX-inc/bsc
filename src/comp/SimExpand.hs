...
simExpandWith errh flags checkTop topname fabis = do
    ...
    let (top_clk, top_rst) =
            simTopClockReset (resetName flags) (abmi_pps topModInfo)
...
--
-- When an argument is designated as the default clock/reset, its own port
-- serves as the top-level one instead, named by the rename pragmas or by
-- prefixing the argument's name.  @dflt_rst_name@ is the reset prefix in
-- force (bsc's -reset-prefix); the caller supplies it because this has to
-- give the same answer in bsc and in the exporter.
simTopClockReset :: String -> [PProp] -> (Maybe String, Maybe String)
simTopClockReset dflt_rst_name top_pps = (top_clk, top_rst)
  where
    ...
    def_rst = case (getDefaultResetArg top_pps) of
                Just arg ->
                    let prefix = fromMaybe dflt_rst_name $
