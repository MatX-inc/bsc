...
-- constructors it leaves Unchanged (ICDef, ICValue, ICLazyPack,
-- ICLazyUnpack) contribute nothing -- and must not be entered at all,
-- since their payloads can be cyclic (knot-tied top-level defs, heap
-- references).
conFTVarSet :: IConInfo a -> VarSet
conFTVarSet (ICDef {}) = vsEmpty
conFTVarSet (ICValue {}) = vsEmpty
conFTVarSet (ICLazyPack {}) = vsEmpty
conFTVarSet (ICLazyUnpack {}) = vsEmpty
