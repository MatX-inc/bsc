...
compilePackage
    ...
    (mctx, pkgsUsedInCtxReduce, declsUsedInCtxReduce)
    ...
    (mod, tcErrors, pkgsUsedInCode, declsUsedInCode)
...
writeABin errh pps flags dumpnames t prefix modstr srcName oqt
    ...
    do
       ...
       let afilename = mkAName (bdir flags) prefix modstr
           ...
           -- serializes them.  Keep asi_sched_order in the thin form:
           -- SimExportIR needs the original per-module node order when a
           -- later -bir/-trs link exports code from these .ba files.
           sched_info_ba
...
compileCDefToIDef errh flags dumpnames symt host_ds ipkg def =
 do
    ...
    (cpkg_chk, tcErrors, _usedPkgs, _usedDecls)
        <- cTypeCheck errh flags symt taken_ids cpkg_ctx
