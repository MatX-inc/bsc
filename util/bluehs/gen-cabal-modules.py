...
EXCLUDE_FILES = {
    ...
    "showrules.hs", "vcdcheck.hs", "fstcheck.hs", "fstscopes.hs",
    "tconcheck.hs", "bluetcl.hs",
}
...
# FSTRead foreign-imports the vendored libfst C reader (and -lz), which the
# cabal library does not compile in; only the fstcheck tool links those
# objects, so exposing it would ship dangling symbols.
EXCLUDE_MODULES = {"ParsecToken", "ParsecLanguage", "ParsecPerm", "FSTRead"}

# Generated by src/comp/update-build-{system,version}.sh.  Keep them in the
# package even when this generator is run from a freshly checked-out tree,
# before those scripts have materialized the source files.
GENERATED_MODULES = {"BuildSystem", "BuildVersion"}
...
def enumerate_modules():
    ...
    return sorted(set(modules) | GENERATED_MODULES)
