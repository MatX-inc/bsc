...
Changes since release 2026.07:

Toolchain
~~~~~~~~~

* The Haskell tools are now built with Cabal (GitHub PR#1084)
  ** Building requires `cabal-install` 3.14 or newer; the Haskell
     library dependencies no longer need to be installed manually
  ** The SMT solvers (Yices and STP) are statically linked into the
     tools; the installation no longer contains an `inst/lib/SAT`
     directory and the executables in `inst/bin` are no longer
     wrapper scripts (GitHub Issue#10, Issue#163, Issue#404)
  ** The executables locate the installation relative to their own
     path (or via `BLUESPECDIR`, as before)
  ** Editor integration via the Haskell Language Server now works
     from the root of the repository with no extra setup
  ** The testsuite can be run via `cabal test` (see the testsuite
     README)
